...
**After the trial**, optional continued access is offered on **[spectyra.ai](https://spectyra.ai)** (same sign-in as setup). You are not charged during the trial; you choose whether to stay on later. **This skill does not handle card payments** — any upgrade happens in the web app.
...
- **Website and cloud API:** `https://spectyra.ai` — production API base is **`https://spectyra.ai/v1`** (same operator as the marketing site; not a third-party domain).
