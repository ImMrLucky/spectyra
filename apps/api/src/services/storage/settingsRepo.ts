...
import type { OrgSettingsRow, ProjectSettingsRow } from "@spectyra/shared";

// Use Row types directly (full DB representation with IDs and timestamps)
// No aliases to avoid collisions - use OrgSettingsRow/ProjectSettingsRow explicitly
...
export async function getOrgSettings(orgId: string): Promise<OrgSettings> {
  ...
  if (!settings) {
    ...
    settings = await queryOne<OrgSettingsRow>(`
      SELECT org_id, data_retention_days, store_prompts, store_responses, store_internal_debug,
             allow_semantic_cache, allowed_ip_ranges, enforce_sso, allowed_email_domains,
             provider_key_mode, created_at, updated_at
      FROM org_settings
 ...
 */
export async function getProjectSettings(projectId: string): Promise<ProjectSettingsRow> {
  let settings = await queryOne<ProjectSettingsRow>(`
...
export async function updateProjectSettings(
  projectId: string,
  updates: Partial<Omit<ProjectSettingsRow, "project_id" | "created_at" | "updated_at">>
